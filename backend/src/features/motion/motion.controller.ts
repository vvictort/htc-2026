...
const MAX_EVENTS_PER_WINDOW = 30;            // max 30 events / user / minute (demo)
const SAME_CATEGORY_COOLDOWN_MS = 5_000;     // same category within 5 s → skip (demo)
...
function checkRateLimit(uid: string, category: string): { allowed: boolean; reason?: string } {
    ...
    if (category === bucket.lastCategory && now - bucket.lastCategoryAt < SAME_CATEGORY_COOLDOWN_MS) {
        return { allowed: false, reason: `Duplicate category '${category}' within cooldown (${SAME_CATEGORY_COOLDOWN_MS / 1000}s)` };
    }

    // Per-window cap
    if (bucket.count >= MAX_EVENTS_PER_WINDOW) {
        return { allowed: false, reason: `Rate limit exceeded (${MAX_EVENTS_PER_WINDOW} events / ${RATE_WINDOW_MS / 1000}s)` };
    }
 ...
 */
export const logMotionEvent = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
        ...
        const classification = await classifyMotion(
        ...
        const shouldNotify =
            classification.threatLevel === "caution" ||
            classification.threatLevel === "danger";
        ...
        if (shouldNotify) {
            ...
            const notification = await Notification.create({
                userId: user._id,
                type:
                    classification.threatLevel === "danger" ? "boundary" : "motion",
                message,
                snapshot: snapshot || undefined,
                details: {
            ...
            if (prefs.email && user.email) {
                sendEmail(
                    user.email,
                    `🍼 BabyWatcher: ${classification.reason}`,
                    buildNotificationEmail(
            ...
            if (prefs.sms && user.phone) {
                sendSms(
                    user.phone,
                    `BabyWatcher ${emoji}: ${classification.reason} (${timeStr})`
 ...
 */
export const getMotionLogs = async (
    req: Request,
    res: Response
): Promise<void> => {
    try {
